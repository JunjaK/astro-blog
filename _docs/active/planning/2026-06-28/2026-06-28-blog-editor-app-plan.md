...
| **D10** | repo 배치 | **단일 repo, 대칭 `blog/` + `editor/`**(워크스페이스 아님). **공유·Claude 소유 자산은 root 유지**(`_docs/ _note/ .claude/ CLAUDE.md .github/ .mise.toml`), 블로그/에디터 요소만 각 폴더로 분리(사용자 명시). **✅ step 0 완료(2026-06)**: 블로그 `git mv`→`blog/`, `main.yml` 경로 재작성(working-directory·아티팩트·캐시키·sparse-checkout `blog/Dockerfile`·docker 컨텍스트 `blog`), blog 빌드 로컬 검증(58p OK). editor publish-api는 `../src/utils/convertLoader.js` 재사용 | 사용자 선택(scope 시각 분리). `.npmrc`는 blog/로 이동(@minpluto 레지스트리는 블로그 전용). `mise`는 root 공유(node24+bun) |
