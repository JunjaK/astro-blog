...
**Status:** 개정 2026-08-18 — 실물 확인으로 D2·D4·D7 이 뒤집혔다. **Task 1~9·11 완료, 7편 배포 대기**

### 진행 상황 (2026-08-18)

| Task | 상태 | 비고 |
|---|---|---|
| 1. GeoJSON 확보·분할 | ✅ | 16개 475KB. **winding 수정 + 원거리 도서 제외** — 아래 |
| 2. 타입 + 도도부현 매핑 | ✅ | `PREFECTURES` 16곳 ↔ geojson 16개 1:1 확인 |
| 3. `useGeoData` | ✅ | `@types/geojson` 실제 타입 사용(`unknown` 제거) |
| 4. `TravelMap` 골격 | ✅ | **축척이 도도부현 fit 으로 교체됨** — §0 D2′ |
| 5. 루트 곡선 + draw-in | ✅ | catmull-rom 렌더 확인. **애니메이션은 구조만 검증, 재생은 미관측** — 아래 |
| 6. 마커 + 툴팁 + 스크롤 + a11y | ✅ | 결함 2건 잡음. **마커가 도시/현 그룹 단위로 교체됨** — §0 D4′ |
| 7. 모바일 two-tap | ✅ | coarse 경로 전수 확인. **원안 전제가 틀려서 방식 교체** — 아래 |
| 8. VisitedList + details + 폴백 | ✅ | **VisitedList 를 별도 export 로 분리** — 아래 |
| 9. 좌표 조달 | ✅ | 파서 + 단축 URL 해석 스크립트. **입도가 도시 그룹당 1개로 축소** — §0 D7′ |
| 10. 검증 스크립트 | ⬜ | 다음 작업 |
| 11. 마이그레이션 | ✅ | **7편** — `14_12-10` + 12/18~23 6편 |
| 12. e2e | ⬜ | |
| 13. 나머지 편 복제 | ⏸ | **저자 지시로 보류.** 26편 전체가 아니라 위 7편만 진행 |

브랜치 **`feat/travel-map`** (base `518f863`), 커밋 15개.
`2535dce`(T1) `9a4efa7`(T2) `77228b2`(T3) `fd88837`(T4~7) `148ac94`(T8) `9827d03`(T9)
`0fc409d`(파일럿) `c45a9db`(도서 제외) `312f3bb`(현 fit + 도시 마커) `fcbfdf5`(6편)
`67b6297`(현 실루엣) `0b3657c`(현 마커) + docs·chore.

T4~7 이 한 커밋인 이유는 `TravelMap.tsx` / `travel-map.css` 가 신규 파일이라 네 태스크가 같은
파일에 누적됐기 때문 — 쪼개면 실제로 테스트한 적 없는 중간 상태가 히스토리에 들어간다.

타입체크 베이스라인: `bun astro check` → **15 errors (전부 기존 코드)**. net-new 0 을 유지한다.

### 🔁 2026-08-18 개정 — 실물이 뒤집은 결정 3건

계획을 세울 때의 전제가 화면에서 틀린 것으로 드러났다. 순서대로 기록한다.

**D2′ 축척: 그날 spots bbox → 도도부현 전체.**
「시정촌 경계가 도시 스케일에서 배경 텍스처를 준다」가 전제였는데 틀렸다. 다카야마시 하나가
2,166km²(일본 최대 면적 시)라 하루 동선 축척에서는 화면 전체가 시 하나 안이고, 그릴 경계선이
없어 흰 캔버스에 루트만 떠 있었다. 현 단위로 빼면 42개 시정촌 경계가 배경으로 살아난다.
부작용: 도도부현 fit 이 되살아나면서 원거리 도서 문제가 부활했다(도쿄도 남북 1,237km).

**D4′ 마커: 개별 장소 → 도시 그룹, 2개 이상 현이면 현 그룹.**
장소 단위로 찍으니 10곳이 유니크 픽셀 7곳에 겹쳐 육안 구분은 3개뿐이었고, 나머지 7개는
**보이지 않는데 Tab 으로는 포커스되는** 접근성 결함이었다. 도시 그룹으로 묶어 해결했고,
다현 편(24_12-20, 700km)에서 도쿄 지구 3개가 다시 겹치자 「배경 입도 = 마커 입도」로 일반화했다.
도쿄만 특수처리하지 않는다.

| | 배경 | 마커 |
|---|---|---|
| 단일 현 | 시정촌 경계 | 도시 |
| 2개 이상 현 | 현 실루엣(내부 경계선 제거) | 현 |

**D7′ 좌표 입도: 장소별 전수 → 도시 그룹당 1개.**
D4′ 로 마커가 그룹 중심점이 되면서 장소별 좌표는 **아무도 쓰지 않는 데이터**가 됐다.
`DiarySpot.lat/lng` 를 옵션으로 만들고 그룹당 대표 하나만 채운다. 좌표가 하나도 없는 그룹은
지도에서 빠지되 목록에는 남는다. 6편 기준 34개 → 13개.

**데이터는 하나도 버리지 않았다.** 지도가 도시/현으로 물러나도 `VisitedList` 는 항상 도시 단위
그룹핑으로 장소 전체를 렌더한다(24_12-20: 마커 2개, 목록은 도시 4그룹·장소 8곳).
...
*(D2′ 이전에는 `spotsExtent()` 가 사각형을 `Polygon` 이 아니라 `MultiPoint` 로 돌려줘 이 함정을 피했다. 지금은 도도부현 fit 이라 그 함수 자체가 없지만, **직접 GeoJSON 을 만들 때는 같은 함정이 살아있다** — 점 집합에는 winding 이 없다는 회피법을 기억할 것.)*

### ⚠️ 원거리 도서 (D2′ 로 부활한 문제)

도도부현 전체에 `fitExtent` 하므로 도쿄도를 그대로 두면 오가사와라·이즈 제도 때문에 **남북 1,237km** 가 잡혀 23구가 점이 된다. `split-muni-geojson.mjs` 의 `ISLAND_MUNICIPALITIES` 로 6곳을 이름으로 제외해 **135km** 로 줄였다.

**거리 임계값 자동판정은 쓸 수 없다.** 니이가타의 `糸魚川市` 는 현 중심에서 1.1° 떨어졌지만 엄연한 본토 도시라 같이 잘려나간다(실측). 16개 현 중 해당되는 건 도쿄도 하나뿐이고, 가고시마·오키나와·시마네를 추가하면 같은 처리가 필요하다.

### ⚠️ 해안선 단순화와 point-in-polygon (Task 10 이 반드시 반영할 것)

s0001(간소화 0.1%)은 해안선이 거칠어서 **해안 장소가 폴리곤 밖으로 떨어진다.** 마쓰시마 해안 좌표가 `松島町` 폴리곤 밖 200m 로 나왔다(가장 가까운 다음 시정촌은 `利府町` 1.93km — 위치 자체는 맞다).

→ 검증은 「폴리곤 안」이 아니라 **「폴리곤 안 또는 경계에서 1.5km 이내」** 로 한다. 엄격한 판정은 해안 편에서 거짓 실패를 낸다.
...
**Architecture:** React island (`client:visible`). D3 는 지리 연산 전용(`geoMercator`, `geoPath`, `line`, `curveCatmullRom`)이고 DOM 은 React 소유. 배경은 시정촌(市区町村) 경계 GeoJSON 을 도도부현 단위로 분할해 `public/geo/muni/{code}.json` 으로 정적 배포하고, **글이 쓰는 현만 fetch** 한다(1편 보통 30~70KB). 축척은 **그 글이 지나는 도도부현 전체**에 맞추고, 마커는 배경 입도에 맞춰 도시 또는 현 단위로 찍는다(§0 D2′·D4′).
...
**전수 파싱 실측 (27편, `01_intro` 는 섹션 없음이 정상):**

| 항목 | 수 |
|---|---|
| 장소(spot) 총계 | **215** |
| 「방문한 곳」 안의 단축 URL = 자동 해석 가능 | **62** (29%) |
| 본문 전체의 단축 URL | 87 — 섹션 밖 23건 중 spot 이름이 같은 줄에 있는 건 **6건**뿐이라 이득이 적다 |
| → 사람이 채워야 하는 좌표 | **약 150** |

파서가 짚어낸 것: 3단 중첩 10편 · 쉼표 포함 항목(사람이 분리) · 도도부현 누락(`27_12-23` 「센다이시」) · `28_12-24` 는 상위 항목만 있고 하위가 없음(나리타/인천 — 사람 판단) · `25_12-21` 은 **하위 항목 안에서 `—` 를 쓰는 4번째 변형**(자동 분해하지 않고 표시만).
...
1. 모든 spot 이 `name` · `city` · `prefecture` 를 갖는가 (`lat`/`lng` 는 D7′ 로 **옵션**이 됐다)
2. **도시 그룹마다 좌표가 최소 1개 있는가** — 하나도 없으면 그 그룹이 지도에서 조용히 사라진다. D7′ 의 핵심 불변식이고, 옛 「`lat === 0` 감지」를 대체한다
3. `prefecture` 가 `PREFECTURES` 에 있는가 — 원문 오타(`이시키와현` 4건) 유입 차단
4. 좌표가 해당 `prefecture` 안에 있는가 — **「폴리곤 안 **또는 경계에서 1.5km 이내**」로 판정한다.** 엄격한 point-in-polygon 은 해안 장소에서 거짓 실패를 낸다(마쓰시마 200m 밖). 이 체크가 Nominatim 오매칭 2건(중국 선전 학교 / 오카야마 구라시키)을 실제로 잡았다
5. `anchor` 가 있으면 해당 MDX 에 그 id 를 만드는 헤딩이 실제로 있는가. **슬러그를 직접 계산하지 말 것** — Astro 가 붙이는 id 가 정답이고 규칙이 미묘하다(`—` 는 사라지고 연속 공백이 `--` 가 된다). 렌더된 HTML 이나 `astro:content` 의 `headings` 에서 읽어 대조한다
6. 마이그레이션한 MDX 가 `spots` 를 import 하고 `<TravelMap>` + `<VisitedList>` 를 쓰는가
...
9. **모든 GeoJSON feature 의 `geoArea` < 1 sr** — winding 이 뒤집힌 파일을 잡는다. 재생성 후 `toD3Winding()` 을 안 거치면 4π 가 나오고 지도가 화면을 덮는 덩어리가 된다
...
**대상 (저자 지시):** 파일럿 `14_12-10` + 12/18~23 구간 6편(`22_12-18` ~ `27_12-23`).
26편 전체 복제는 보류(Task 13). 12/18~23 을 고른 건 **서사 소제목이 가장 많은 구간**이라
(4·11·8·9·9·6개) 파일럿에서 검증 못 한 클릭→본문 스크롤을 여기서 확인할 수 있기 때문이다.
...
### 루트
<TravelMap spots={spots} originalImageSrc="…{그 편의 루트 스크린샷}.webp" client:visible />

### 방문한 곳
<VisitedList spots={spots} />          {/* client:* 없음 — 정적 */}
```

**`<TableOfContents>` 는 손대지 않는다.** 원안은 `방문한 곳` 줄을 빼라고 했지만, `VisitedList` 를
그 헤딩 아래에 두면 두 헤딩이 그대로 남아 목차가 어긋나지 않는다. 27편의 목차를 건드릴 이유가 없다.

**⚠️ 헤딩 매칭 주의.** `## 루트 및 방문한 곳` 이 있으므로 `/^#+\s*루트/` 로 찾으면 그 줄이 먼저
잡혀 `### 루트` 안의 스크린샷을 놓친다(실제로 겪었다). `/^#{2,6}\s+루트$/` 처럼 **끝을 고정**한다.

### 좌표 출처와 검증 (7편, 도시 그룹 13곳 + 파일럿 3곳)

| 출처 | 수 | 비고 |
|---|---|---|
| 구글맵 단축 URL | 3 | 파일럿에만 있다. 12/18~23 6편은 URL 0건 |
| OSM Nominatim | 10 | 일본어 명칭으로 조회하고 결과 주소의 시정촌까지 대조 |
| 저자 제공 | 4 | 직접 확인한 좌표 3건 + 마쓰시마 Plus Code 1건 |

**전수 point-in-polygon 검증. 오매칭 2건을 여기서 잡았다:**
...
Nominatim 결과를 그대로 믿으면 지도가 **조용히** 틀린다. `countrycodes=jp` 만으로는 부족하고
(`平湯神社` 는 jp 제한 후에도 오사카·사가·아키타의 엉뚱한 신사를 반환했다), 폴리곤 대조가 필요하다.

**Plus Code 디코드.** 저자가 준 `9397+VM 마쓰시마마치` 같은 short code 는 기준점이 필요하다.
임의 좌표를 기준으로 쓰지 말고 **해당 시정촌 폴리곤의 중심점**(`geoCentroid`)을 쓰면 검증 가능한
값만으로 복원된다. 복원 결과 `8RC39397+VM` → 38.3696875, 141.0641875.

### 원문 데이터 정정 (저자 확인)

| 편 | 원문 | 정정 | 근거 |
|---|---|---|---|
| 22·23 | `아오모리현 후쿠우라지마` | 후카우라마치 | 不老ふ死温泉 은 深浦町 소재. OSM 주소로 확인 |
| 23 | `아오모리현 이시에시` | 아오모리시 | 石江 는 아오모리시 지구. 新青森駅 주소가 「石江, 青森市」 |
| 26 | `미야기현 센다이 시` | 센다이시 | 띄어쓰기 통일 |
| 27 | `센다이시 ` | 미야기현 센다이시 | 도도부현 누락 + 뒤 공백 |
| 27 | `마츠시마 가이칸` | 미야기현 마쓰시마쵸 | 松島海岸. 도도부현도 도시명도 아니었다 |
| 25 | 도쿄 지구가 하위 항목 | 지구를 상위로 | 안 올리면 도쿄에 마커 1개. 24편이 이미 그 구조 |

### 검증 결과 (2026-08-18 실측)

| 편 | 마커 | 유니크 위치 | 목록 | 배경 | fetch |
|---|---|---|---|---|---|
| 14_12-10 | 3 | 3 | 3 | 시정촌 | 21 |
| 22_12-18 | 1 | 1 | 1 | 시정촌 | 02 |
| 23_12-19 | 4 | 4 | 4 | 시정촌 | 02 |
| 24_12-20 | 2 | 2 | **4** | **현 실루엣** | 02 + 13 |
| 25_12-21 | 5 | 5 | 5 | 시정촌 | 13 |
| 26_12-22 | 1 | 1 | 1 | 시정촌 | 04 |
| 27_12-23 | 2 | 2 | 2 | 시정촌 | 04 |

**전편 겹침 0** (마커 수 == 유니크 픽셀 위치 수). 24_12-20 이 마커 2 / 목록 4 인 것은 의도다 —
지도는 현 단위로 물러나고 목록은 도시 상세를 유지한다.

**클릭→본문 스크롤 확인.** `23_12-19` 의 히로사키시 마커 클릭 → `#히로사키성-첫번째-스탬프` 로
스크롤, 헤딩이 뷰포트 80px 에 위치해 sticky 네비(약 64px)에 **가려지지 않는다.**
→ 우려했던 `scroll-margin-top` 은 사이트가 이미 처리하고 있었다. 추가 작업 불필요.

### 남은 한계

- `14_12-10` 의 미확인 좌표 3곳(키타 공원 · 히라유 신사 · 히라유 온센 아시유)이 주석 처리 상태다.
  OSM 에도 없고 본문에도 링크가 없다. 지도에는 영향 없지만(도시 그룹 좌표는 확보) 목록에서 빠졌다.
- `14_12-10` 은 `## 일정` 에 소제목이 없어 anchor 가 없다(27편 중 9편이 그렇다). 클릭은 툴팁만.

---

## Task 12: Playwright e2e

- [ ] **Step 1:** `blog/e2e/travel-map.noauth.spec.ts`

**대상 2편을 쓴다** — 한 편으로는 두 모드를 다 못 덮는다.
`23_12-19`(단일 현 · 도시 마커 4개 · anchor 있음) + `24_12-20`(2개 현 · 현 마커 2개 · 실루엣).

케이스:
1. `circle.tm-dot` 개수 == `groupSpots(spots, level)` 결과 길이 — 데이터 파일과 그룹핑 함수를 test 가 직접 import 해 비교한다. **숫자를 하드코딩하지 않는다**
2. 마커 수 == 유니크 `cx,cy` 위치 수 (겹침 0 회귀 방지 — D4′ 가 고친 결함이다)
3. (chromium) 마커 hover → `[role="tooltip"]` 에 그 그룹 라벨
4. (chromium) anchor 가 있는 마커 클릭 → 대상 헤딩이 뷰포트 안 **이고 상단 64px 아래**(sticky 네비에 가리지 않음)
5. `구글 맵 원본 보기` 클릭 → 원본 이미지 표시
6. 「방문한 곳」 목록 그룹 수가 **도시 단위** 기준과 일치 (24_12-20 은 마커 2 / 목록 4 — 지도 입도와 목록 입도가 다른 것이 정상임을 고정)
7. `24_12-20` 에 `.tm-muni--plain` 이 있고 `23_12-19` 에는 없다
8. (mobile-chrome) 1탭 = 툴팁만, 같은 마커 2탭 = 스크롤
9. 접근성: 모든 마커가 `role="button"` + `tabIndex=0` 이고 `<svg>` 는 `role="group"` (`role="img"` 회귀 방지 — 하위 트리가 presentational 이 되면 마커가 AT 에서 사라진다)
...
**저자 지시로 보류한다.** 26편 전체가 아니라 Task 11 의 7편만 진행한다. 재개할 때 아래를 쓴다.

착수 전에 확인할 것: **`PREFECTURES` 에 없는 도도부현이 나오면 먼저 `split-muni-geojson.mjs` 의
`KEEP` 에 코드를 넣고 재실행**해야 한다. 타입은 통과하는데 fetch 가 404 나는 함정을 막기 위해
매핑에 있는 이름은 반드시 GeoJSON 파일이 있어야 한다는 불변식을 유지하고 있다.
